...
$active_services = countRow(["table"=>"services"]);
$active_tickets = countRow(["table"=>"tickets"]);
$active_clients = countRow(["table"=>"clients"]);
$stylesheet = [
    "stylesheets" => [
        "/assets/cj7gone/simplify/bootstrap.css",
    ...
    "scripts" => [
        ...
        "/assets/007twg/js/ch3915babussofa4.js",
        "/assets/007twg/js/cgtptn05b64bwcs4.js",
        "/assets/007twg/js/xcz59lmywkfdgsp4.js",
        "/assets/007twg/js/wnzsoolloslhfumj.js",
        "/assets/007twg/js/qri1ac173hshc2hdjc.js",
        "/assets/007twg/js/script.js",
        "/assets/007twg/js/newyear.js",
        "/assets/007twg/js/custome.js",
    ],
];
  ...
  if ($_SESSION["msmbilisim_userlogin"] != 1 || $user["client_type"] == 1) {
    echo $twig->render(
      ...
      array(
        ...
        'whatsappbutton' => $settings["whatsappbutton"],
        'whatsappnumber' => $settings["whatsappnumber"],
        'whatsappcolour' => $settings["whatsappcolour"],
        'whatsappposition' => $settings["whatsappposition"],
        'telegrambutton' => $settings["telegrambutton"],
        'telegramusername' => $settings["telegramusername"],
        'telegramcolour' => $settings["telegramcolour"],
        'telegramposition' => $settings["telegramposition"],
        'status' => $route[1],
        'earn' => $earnList,
        'orders' => $ordersList,
        ...
        'timezones' => $timezones,
        'Allt'=>$active_tickets,
        'Alls'=>$active_services,
        'Allc'=>$active_clients,
        'totalorders'=>$order_count,
        'psw_img' => $settings["theme_bg_img"],
        'panel' => $panel,
        'general' => $general,
        'updates' => $updates,
        'blogs' => $blogs,
    ...
    if ($statusData["total"] <= $bronze_status) {
    $status = "VIP";
}

if ($statusData["total"] > $bronze_status && $statusData["total"] <= $silver_status) {
    $status = "JUNIOR";
}

if ($statusData["total"] > $silver_status && $statusData["total"] <= $gold_status) {
    $status = "REGULAR";
}

if ($statusData["total"] > $gold_status && $statusData["total"] <= $dealer_status) {
    $status = "NEW";
}

if ($statusData["total"] > $dealer_status) {
    $status = "PREMIUM";  // Assuming this is the intended highest status
}
    ...
    echo $twig->render(
      ...
      [
        ...
        "myreview" => $myreview,
        "mode" => $mode,
        "free_service_html"=> $free_service_html,
        "free_service_html_simplify"=> $free_service_html_simplify,
